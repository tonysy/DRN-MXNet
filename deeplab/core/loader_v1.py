...
class TrainDataLoader(mx.io.DataIter):
    ...
    def get_batch_parallel(self):
        ...
        for idx, islice in enumerate(slices):
            ...
            try:
                multiprocess_results.append(self.pool.apply_async(parfetch, (self.config, 
                self.crop_width, self.crop_height, isegdb)))
            except:
...
def parfetch(config, crop_width, crop_height, isegdb):
    ...
    if config.dataset.dataset == "PascalVOC" or config.dataset.dataset == "ADE20K" :
        ...
        if config.TRAIN.enable_crop:
            ...
            sx = (int)(sx)
            sy = (int)(sy)
            # print("sx:", datas_internal.shape)
            # print("xxx", crop_height)
            # print("xxx", crop_width)
        ...
        # print("Fetch",datas.shape,labels.shape)
        return {'data': datas, 'label': labels}
